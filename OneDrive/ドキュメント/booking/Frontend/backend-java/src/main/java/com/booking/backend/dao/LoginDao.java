...
public class LoginDao {
    ...
    private static final String DEFAULT_ADMIN_EMAIL = "kumarsaripalli1198@gmail.com";
    private static final String DEFAULT_ADMIN_PASS = "admin1234";

    public boolean validateAdmin(String identity, String password) {
        String normalized = identity == null ? "" : identity.trim().toLowerCase();
        String adminUser = envOrDefault("ADMIN_USER", DEFAULT_ADMIN_USER).toLowerCase();
        String adminEmail = envOrDefault("ADMIN_EMAIL", DEFAULT_ADMIN_EMAIL).toLowerCase();
        String adminPass = envOrDefault("ADMIN_PASS", DEFAULT_ADMIN_PASS);

        return (adminUser.equals(normalized)
                || adminEmail.equals(normalized)
